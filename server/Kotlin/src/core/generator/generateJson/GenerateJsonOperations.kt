...
class GenerateJsonOperations {
    ...
    fun generateTokenAndGenrateJsonData(dataLines: List<String>): String{
        var tokenList = generateToken(dataLines)
    ...
    private fun generateToken(dataLines: List<String>): List<ComplexToken> {

        var tokenList: MutableList<ComplexToken> = mutableListOf()
        for ((lineNumber, aLine) in dataLines.withIndex()) {
            ...
            Println.purple("key value  $keyValuePairPlusMetaForOriginalMap")
         ...
         val   tokenItem = ComplexToken(
            ...
            Println.yellow("======================================================")
            Println.yellow("Token $tokenItem")
            Println.yellow("======================================================")
    ...
    fun validateGeneratorHints(lineHint: String, lineNumber: Int): Pair<Boolean, String> {
        ...
        for ((wordNumber, word) in splitByComma.withIndex()) {
            ...
            if (generatorHints.contains(word)) {
                ...
                if (word.contains("=")) {
                    val startIndex = word.substring(word.indexOf("["), word.indexOf("="))
                    val endIndex = word.substring(word.indexOf("="), word.indexOf("]"))
                    Println.yellow("range string $word from $startIndex to $endIndex")
                    try {
                        val startIndexInt = startIndex.toInt()
                        val endIndexInt = endIndex.toInt()
                        Println.yellow("user choose a range of $startIndexInt and $endIndexInt")
